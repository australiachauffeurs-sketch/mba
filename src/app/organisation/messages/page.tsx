              ...
              filtered.map(conv => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-slate-50 transition-colors border-b border-slate-50 ${
                    selectedUser?.id === conv.partner.id ? "bg-indigo-50 border-l-2 border-l-indigo-500" : ""
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      ...
                      {conv.unread > 0 && (
                        <span className="ml-1 flex-shrink-0 bg-indigo-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
                          {conv.unread}
                        </span>
                      )}
                    </div>
            ...
            <>
                  ...
                  messages.map(msg => {
                    ...
                    return isSent ? (
                      <div key={msg.id} className="flex justify-end mb-2">
                        <div className="max-w-xs bg-indigo-600 text-white text-sm px-4 py-2 rounded-2xl rounded-tr-sm">
                          {msg.content}
                          <div className="text-xs text-indigo-200 mt-1 text-right">{formatTime(msg.created_at)}</div>
                        </div>
                      </div>
                    ) : (
              ...
              <div className="px-6 py-4 bg-white border-t border-slate-200 flex items-center gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-2 text-sm border border-slate-200 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
                <Button
                  onClick={sendMessage}
