...
import type { AIRecommendationsResult } from "@/app/api/ai/alumni-recommendations/route"

export default function AlumniAIAdvisorPage() {
  const [data, setData] = useState<AIRecommendationsResult | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(false)

  useEffect(() => {
    fetch("/api/ai/alumni-recommendations")
      .then(r => r.json())
  ...
  if (loading) return (
    <>
      ...
      <main className="flex-1 flex flex-col items-center justify-center gap-3 min-h-[60vh]">
        ...
        <p className="text-sm text-slate-500">AI is analysing your profile…</p>
      </main>
    </>
  )
          ...
          <div key={cat.type} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {cat.items.map((item, i) => (
                <Card key={i} className={item.urgent ? "ring-2 ring-indigo-300" : ""}>
                  <CardContent className="p-4 space-y-3">
                    {item.urgent && (
                      <span className="flex items-center gap-1 text-xs bg-indigo-600 text-white px-2 py-0.5 rounded-full w-fit">
                    ...
                    <div className="bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2">
                      <p className="text-xs text-indigo-800"><span className="font-semibold">Why you: </span>{item.why}</p>
                    </div>
