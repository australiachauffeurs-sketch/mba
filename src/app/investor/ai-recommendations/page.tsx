...
import type { AIRecommendationsResult } from "@/app/api/ai/investor-recommendations/route"

export default function InvestorAIAdvisorPage() {
  const [data, setData] = useState<AIRecommendationsResult | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(false)

  useEffect(() => {
    fetch("/api/ai/investor-recommendations")
      .then(r => r.json())
  ...
  if (loading) return (
    <>
      <Topbar title="AI Advisor" subtitle="Personalised deal flow and founder recommendations" />
      <main className="flex-1 flex flex-col items-center justify-center gap-3 min-h-[60vh]">
        <Loader2 className="w-8 h-8 animate-spin text-amber-500" />
        <p className="text-sm text-slate-500">AI is analysing your investment thesis…</p>
      </main>
    </>
  )
          ...
          <div key={cat.type} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {cat.items.map((item, i) => (
                <Card key={i} className={item.urgent ? "ring-2 ring-amber-300" : ""}>
                  <CardContent className="p-4 space-y-3">
                    {item.urgent && (
                      <span className="flex items-center gap-1 text-xs bg-amber-500 text-white px-2 py-0.5 rounded-full w-fit">
                    ...
                    <div className="bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
                      <p className="text-xs text-amber-800"><span className="font-semibold">Why you: </span>{item.why}</p>
                    </div>
